...
export class CategoriasService {
    ...
    async criarCategoria(criarCategoriDto: CriarCategoriaDto): Promise<Categoria> {
        ...
        await this.findCategory(categoria);
    ...
    async consultarCategoriaPeloId(categoria: string): Promise<Categoria> {
        return await this.findCategory(categoria);
    }
    ...
    async atribuirJogador(params :string[]): Promise<void> {
        ...
        const categoriaEncontrada = await this.findCategory(categoria);

        const jogadorAtribuido = await this.categoriaModel
            .find({categoria})
            .where('jogadores')
            .in(idJogador)
            .exec()

        if (jogadorAtribuido.length > 0) {
    ...
    async findCategory(categoria: string): Promise<Categoria> {
